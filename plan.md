...
Phase 12.10（CI、App 构建运行与 GitHub Actions 验收）已完成；最新代码提交 `93630a3` 的 GitHub Actions 两个 job 均已实际成功。本轮未扩展删除、隔离、相似媒体或网络功能；原生 App 图标 Bundle 已完成可用性与 CI 验收。
...
- 原生 App 图标：新增深蓝—青绿的硬盘索引标志，PNG 源文件覆盖 16px 至 1024px。初版 Xcode Asset Catalog 生成的 Bundle 图标只有 110KB，且自动生成的 Info.plist 未实际写入 `CFBundleIconFile`，会显示 macOS 通用图标。现改为由 `iconutil` 验证的完整 1.96MB `DiskIndexer.icns`，作为 Resources 资源随 Bundle 复制；新增受版本控制的 Info.plist 显式声明 `CFBundleIconFile=DiskIndexer.icns`。CI 的 `macos-app` 会提取并检查 Info.plist、图标文件和全部 10 个标准尺寸，防止回归。不改变任何扫描、清理或网络边界。提交 `93630a3` 的 GitHub Actions 运行 `30451436913` 中 `quality` 与 `macos-app` 均已实际成功。
